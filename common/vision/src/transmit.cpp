...
void int_to_bytes(int n, char *bytes, int &a) 
{	
   	bytes[a+0] = (n >> 24) & 0xFF;
	bytes[a+1] = (n >> 16) & 0xFF;
	bytes[a+2] = (n >> 8) & 0xFF;
...
void transmit::send(std::vector<int> centre_xs)
{
	char buff[1024];
	...
	for (int i = 0; i < 1023; i++)
		std::cout << (int)buff[i] << ' ';
	sendto(sockfd, buff, 1024, 0, (sockaddr *)&addr, sizeof(addr));
}
